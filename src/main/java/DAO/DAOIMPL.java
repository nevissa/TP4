...
public class DAOIMPL implements IGENERIC {
	...
	public List<ElectronicDevices> getElectronic() {
		List<ElectronicDevices> listDevices = manager
				.createQuery("Select h From ElectronicDevices h", ElectronicDevices.class).getResultList();
		return listDevices;
	}
	...
	public List<Home> getHomeParPersonne(int OWNER_ID) {
		List<Home> listHomeParPers = manager
				.createQuery("select home from Home home where OWNER_ID=OWNER_ID", Home.class).getResultList();
